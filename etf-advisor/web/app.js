...
function PCPremium({
  ...
  if (selFund) {
    ...
    const metrics = [['净值', selFund.nav.toFixed(3), null], ['估算净值', estNav.toFixed(3), chg(fc?.ratio_pct)], ['涨幅', fmtPct(selFund.change), chg(selFund.change)], ['估算溢价', fmtPct(selFund.display_premium), chg(selFund.display_premium)], ['3M均溢价', fmtPct(selFund.avg_3m), null], ['6M均溢价', fmtPct(selFund.avg_6m), null], ['1Y均溢价', fmtPct(selFund.avg_1y), null], ['年净值涨幅', fmtPct(selFund.nav_return_1y), chg(selFund.nav_return_1y)], ['年价格涨幅', fmtPct(selFund.price_return_1y), chg(selFund.price_return_1y)], ['溢价>7%天数', String(selFund.days_gt7), selFund.days_gt7 > 30 ? '#A8342A' : null], ['分值', selFund.score.toFixed(2), null]];
...
function MobPremium({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    }, [['净值', e.nav.toFixed(3), null], ['估算净值', estNav.toFixed(3) + ' (' + fmtPct(fc?.ratio_pct) + ')', chg(fc?.ratio_pct)], ['估算溢价', fmtPct(e.display_premium), chg(e.display_premium)], ['3M均溢价', fmtPct(e.avg_3m), null], ['6M均溢价', fmtPct(e.avg_6m), null], ['1Y均溢价', fmtPct(e.avg_1y), null], ['年净值涨幅', fmtPct(e.nav_return_1y), chg(e.nav_return_1y)], ['年价格涨幅', fmtPct(e.price_return_1y), chg(e.price_return_1y)], ['溢价>7%天数', String(e.days_gt7), e.days_gt7 > 30 ? '#A8342A' : null]].map(([l, v, c]) => /*#__PURE__*/React.createElement("div", {
      key: l,
      style: {
        display: 'flex',
        justifyContent: 'space-between',
